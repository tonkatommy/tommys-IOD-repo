...
module.exports.get_filter = (req, res) => {
  ...
  if (matchingFriends.length > 0) {
    ...
    return res.status(404).json({ error: "No friends matching gender " + filterGender });
  }
};
...
module.exports.get_filterLetter = (req, res) => {
  ...
  let filterLetter = req.params.letter.toLowerCase();
  let matchingFriends = [...friends];

  if (filterLetter) {
    matchingFriends = matchingFriends.filter((friend) => friend.name[0].toLowerCase() == filterLetter);
  }
...
module.exports.get_id = (req, res) => {
  ...
  if (foundFriend) {
    ...
    res.status(200).json({ result: `Found friend with ID ${friendId} :  ${foundFriend.name}, ${foundFriend.gender} ` });
  } else {
...
module.exports.put_updateById = (req, res) => {
  ...
  if (foundFriend) {
    ...
    res.status(200).json({ result: "Updated friend with ID " + friendId, data: foundFriend });
  } else {
    res.status(404).json({ error: "No friends matching id " + friendId });
  }
};
